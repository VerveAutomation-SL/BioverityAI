...
const rangeFromParams = (
  ...
  if (type === "day") {
    ...
    return { start: dayRange(from, timeZone).start, end: dayRange(to, timeZone).end, label: `${from} to ${to}`, type: "day" };
  }
  ...
  if (type === "week") {
    ...
    return { start: dayRange(start, timeZone).start, end: dayRange(end, timeZone).end, label: `Week ${weekNum}, ${year}`, type: "week" };
  }
  ...
  if (type === "month") {
    ...
    return { start: dayRange(start, timeZone).start, end: dayRange(end, timeZone).end, label: `${monthNames[+m - 1]} ${y}`, type: "month" };
  }

  if (type === "year") {
    const year = params.get("year");
    if (!year) throw new Error("Missing year parameter");
    return { start: dayRange(`${year}-01-01`, timeZone).start, end: dayRange(`${year}-12-31`, timeZone).end, label: year, type: "year" };
  }
...
export async function GET(req: Request) {
  try {
    ...
    const { data: profile } = await supabase
      .from("profiles").select("org_id, organization_logo, country").eq("id", user.id).single();
    ...
    const { start, end, label, type } =
      rangeFromParams(searchParams, orgTimezone);
    ...
    const drawPageFooter = (pg: ReturnType<PDFDocument["addPage"]>, pageNum: number, total: number) => {
      ...
      pg.drawText(
        `Generated by BioVerity AI  ·  bioverityai.com  ·  ${formatLocalDateTime(new Date(), orgTimezone)}  ·  Working Hours: ${schedule.morning_start}–${schedule.morning_end}, ${schedule.evening_start}–${schedule.evening_end}   |   Page ${pageNum} of ${total}`,
    ...
    for (let idx = 0; idx < (employees?.length ?? 0); idx++) {
      ...
      if (type === "day") {
        ...
        if (empLogs.length > 0) {
          ...
          checkIn  = formatLocalTimeFromISO(
            empLogs[0].toISOString(),
            orgTimezone
          );

          checkOut = formatLocalTimeFromISO(
            empLogs[empLogs.length - 1].toISOString(),
            orgTimezone
          );
        }
