...
export async function GET(req: Request) {
    try {
        ...
        const C = {
            ...
            white:     rgb(1, 1, 1),
            dark:      hex("#212529"),
            headerBg:  hex("#f1f3f5"),     // ✅ greyish
            headerTxt: hex("#495057"),     // dark grey for text
            green:     hex("#198754"),
            purple:    hex("#6f42c1"),
            cyan:      hex("#0dcaf0"),
            rowAlt:    hex("#f0f4ff"),
        ...
        // Common header function for both pages
        const drawPageHeader = async (pg: ReturnType<PDFDocument["addPage"]>, title: string) => {
            ...
            // ✅ Org logo or avatar (RIGHT, with org name below)
        ...
        const zone1X = M;
        const zone1W = (pageW - M * 2 - 20) / 2;   // equal width
        ...
        [
            ...
            const lx = pie1CX - 55;
            const ly = legY - i * 18;
            rect(page1, lx, ly, 11, 11, l.colour);
            page1.drawText(l.label, { x: lx + 15, y: ly + 1, size: 10, font: fontReg, color: C.dark });
        });

        const t1 = "Present vs Absent";
        const t1W = fontBold.widthOfTextAtSize(t1, 11);
        page1.drawText(t1, { x: zone1X + (zone1W - t1W) / 2, y: chartBot - 10, size: 11, font: fontBold, color: C.dark });
        ...
        // Legend
        const legX = zone2X;
        rect(page1, legX,      chartY - 6, 10, 10, C.green);
        page1.drawText("Present", { x: legX + 14, y: chartY - 6, size: 8, font: fontReg, color: C.dark });
        rect(page1, legX + 65, chartY - 6, 10, 10, C.danger);
        page1.drawText("Absent",  { x: legX + 79, y: chartY - 6, size: 8, font: fontReg, color: C.dark });

        drawBarChart(page1, zone2X, chartBot + 20, zone2W, chartH - 40, deptBars, deptMaxVal, fontReg, 9, C.green, C.danger);
        ...
        records.forEach((rec, idx) => {
            ...
            rect(currentPage, tableX, rowY - rowH, 3, rowH, rec.status === "Present" ? C.green : C.danger);
